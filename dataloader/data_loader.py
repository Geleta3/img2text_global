...
def get_loader(transform,
    ...
    Args:
      ...
      specific_folder: If you want to test images in a any folder.
    """
    # path = os.getcwd()
    path = config.image_path
    if mode == "test" and specific_folder is not None:
        path = specific_folder

    assert mode in ['train', 'valid', 'test'], "mode must be one of 'train', 'valid' or 'test'."
    if not vocab_from_file: assert mode == 'train', "To generate vocab from captions file, must be in training mode (" \
                                                    "mode='train'). "

    # Based on mode (train, val, test), obtain img_folder and annotations_file.
    if mode == 'train':
        if vocab_from_file: assert os.path.exists(
            vocab_file), "vocab_file does not exist.  Change vocab_from_file to False to create vocab_file."
        img_folder = os.path.join(path, 'train2017')
        annotations_file = os.path.join(path, 'annotation', 'annotations', 'captions_train2017.json')

    elif mode == 'valid':
        assert os.path.exists(vocab_file), "Must first generate vocab.pkl from training data."
        assert vocab_from_file == True, "Change vocab_from_file to True."
        img_folder = os.path.join(path, 'val2017')
        annotations_file = os.path.join(path, 'annotation', 'annotations', 'captions_val2017.json')
        ...
        if specific_folder is None:
            # Takes from coco.
            img_folder = os.path.join(path, 'test2014')
    ...
    if mode == 'train' or mode == 'valid':
        ...
        data_loader = data.DataLoader(dataset=dataset,
                                      ...
                                      # collate_fn=lambda x: x if mode == "valid" else None,
...
class CoCoDataset(data.Dataset):
    ...
    def __init__(self, transform, mode, batch_size, vocab_threshold, vocab_file, start_word,
                 end_word, unk_word, annotations_file, vocab_from_file, img_folder, specific_folder=None):
        ...
        if self.mode == 'train' or self.mode == 'valid':
            ...
            if specific_folder is not None:
                self.paths = os.listdir(specific_folder)
